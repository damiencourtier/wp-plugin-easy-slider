...
class Easy_Slider_Functions{
    ...
    public static function getOneSlider($slug,$key){
        global $wpdb;
        return $wpdb->get_row("SELECT * FROM " . $wpdb->prefix . "es_sliders WHERE ".$key." = '".sanitize_key($slug)."'");
    }
    ...
    public static function getItemsSlider($slider_id){
        global $wpdb;
        $result = $wpdb->get_results("SELECT * FROM " . $wpdb->prefix . "es_sliders_items WHERE id_slider = ".sanitize_key($slider_id)." ORDER BY order_item ASC");
    ...
    public static function getItemsSliderBySlug($slug){
        global $wpdb;
        $result = $wpdb->get_results("SELECT si.* FROM " . $wpdb->prefix . "es_sliders_items si LEFT JOIN " . $wpdb->prefix . "es_sliders s ON si.id_slider = s.id_slider WHERE s.slug = '".sanitize_key($slug)."' ORDER BY si.order_item ASC");
        return ($result === NUlL?[]:$result);
    }
    ...
    public static function generateSlug($slug , $new = true, $slider_id = null,$index = 0){
        ...
        if(!$new){
            $where = "AND id_slider != " . $slider_id;
        }

        $result = $wpdb->get_var("SELECT COUNT(*) FROM " . $wpdb->prefix . "es_sliders WHERE slug = '" . $slug . "' ".$where);
        if ($result) {
            $index++;
            $slug = $slug.'-'.$index;
